...
#By target risk
findEfficientFrontier.Risk = function(mean_ret, cov_matrix, target_risk, numTicker){
...
findEfficientFrontier.ReturnALT = function(mean_ret, cov_matrix, target_ret, numTicker){
...
calcPortReturn = function(df, from, to, wght, rebalance, geometric = TRUE){
  ...
  port_ret = Return.portfolio(df_range, weights = wght, geometric = TRUE, rebalance_on = reb_op)
